...
def on_join(data):
    ...
    organization_id = session.get(
        'organization_id') if 'user_id' in session else None
    logger.info(
        f"User {user_id} attempting to join session {session_id} (SID: {request.sid}) at {datetime.now().isoformat()} in organization {organization_id}"
    )
    ...
    if not session_data or str(session_data['_id']) != session_id:
        emit('joined_session', {
            'status': 'error',
            'message': 'Session not found or invalid'
        })
        logger.warning(
            f"Failed to join session {session_id} by user {user_id} at {datetime.now().isoformat()}"
        )
        return
    if session_data.get('end_time'):
        ...
        logger.info(
            f"Rejected join for ended session {session_id} by user {user_id}")
        return
    emit(
        'joined_session', {
            'status': 'success',
    ...
    logger.info(
        f"User {user_id} successfully joined session {session_id} at {datetime.now().isoformat()}, rooms: {rooms()}"
    )
...
def handle_live_frame(data):
    try:
        ...
        if not all([user_id, session_id, frame, timestamp]):
            logger.error(f"Invalid live_frame data: {data}")
            return
        ...
        if not session_data:
            logger.warning(
                f"Rejected frame for unknown session: {session_id} in organization {organization_id}"
            )
            return
        ...
        if frame_decoded is not None:
            frame_decoded = cv2.resize(frame_decoded, (1280, 720),
                                       interpolation=cv2.INTER_LINEAR)
            frame_queues[session_id].append(frame_decoded)
            logger.info(f"Received and decoded frame for session {session_id}, size: {len(frame)} bytes, shape: {frame_decoded.shape}, rooms: {rooms()}, clients: {socketio.server.manager.rooms['/'][session_id] if session_id in socketio.server.manager.rooms['/'] else 'No clients'}")
            # Save a sample frame for debugging
            with open(f"server_frame_{session_id}.jpg", 'wb') as f:
                f.write(frame_data)
            logger.info(f"Saved server frame for session {session_id}")
        else:
            logger.error(f"Failed to decode frame for session {session_id}")
            return
        logger.debug(
            f"Broadcasting live frame for session {session_id}, frame_size: {len(frame)} bytes, timestamp: {timestamp}, to rooms: {rooms()}, clients: {socketio.server.manager.rooms['/'][session_id] if session_id in socketio.server.manager.rooms['/'] else 'No clients'}"
        )
        emit('live_frame', {
            'user_id': user_id,
            'session_id': session_id,
...
if __name__ == '__main__':
    socketio.run(app,
                 host='0.0.0.0',
                 port=5001,  # Changed to port 5001
                 debug=True,
